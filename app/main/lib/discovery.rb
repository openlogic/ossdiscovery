# LEGAL NOTICE
# -------------
# OpenLogic Discovery is a tool that finds installed open source software.
...
# (discovery2-client/license/OSSDiscoveryLicense.txt)
# for more details.
#
# You should have received a copy of the GNU Affero General Public License along
# with this program.  If not, see http://www.gnu.org/licenses/
#
# You can learn more about Discovery, report bugs and get the latest versions at
# www.ossdiscovery.org.
...
#
# Every property from the config.yml file is loaded as an instance variable of
# global self.
# This is done so that this file can have default values for all of these
# properties, and then # change them if necessary based on a cli option that
# was specified.  So, if a default value in the config.yml file is ever
# modified, this file will receive that modified value by default.
# The same will happen if a new value is ever added to the config.yml.
#
# Quick and dirty code architecture discussion:
#  1) A Walker is a class which traverses the disk looking for files that match
#     a given set of of regular expressions
#     a) The Walker derives the list of file matches it should be looking for
#        from the RuleEngine
#  2) The RuleEngine is initialized through reading the set of project rules xml
#     files found in the lib/rules directory and subdirectories.  A project rule
#     and its match rules defines filename regular expressions which could
#     indicate a project is installed
#  3) The Walker looks for any file which matches one of rule's "files of
#     interest" (FOI)
#       a) A file of interest is really a regular expression that could match
#          any number of possible files the rule could apply to.
#       b) You can see the list of patterns that make up the "files of interest"
#          by running:
#          ./discovery --list-foi
#  4) When the Walker finds a matching file, it calls the RuleEngine with the
#     found file
#  5) The RuleEngine will evaluate the file and apply any rule which matches
#     that filename
#     a) There are currently 4 types of match rules depending upon the rule
#     writer's judgement for the best way to detect the project's key files.
#  6) The RuleEngine will track the match state for each of the project rules
#     and match rules
#  7) After the Walker has completed the disk traverse, the RuleEngine contains
#     the match states of everything found
#  8) The Discovery framework then dumps a list of the match states to the
#     console in a sorted order
#     a) optionally, the results will be delivered to an open source census
#        server for inclusion in the open source census.
#     For more details on how the project rules work, please see the
#     "Rule Writing for Discovery" document on the project web site:
...
class Discovery
  # don't check for network connectivity more than once
  @@already_checked_connectivity = false

  # don't check for updates more than once
  @@already_checked_for_updates = false

  @@plugin_prepped = false

  attr_accessor :speaker, :dont_open_discovered_archives, :examine_source_files,
    :examine_windows_binary_files, :dont_update_rules, :rule_types,
    :send_results, :throttling_enabled, :always_open_class_file_archives,
    :olex_machine_results_file, :olex_human_results_file,
    :olex_rollup_results_file, :olex_mif_results_file, :show_rollup_report,
    :plugins_list, :rule_engine, :walker
  ...
  def say(message, newline = true)
    if @speaker
      @speaker.say(message, newline)
    else
      # special check - if we're just saying ".", it's a progress message and
      # we won't append a new line
      if message == "."
        putc "."
      elsif newline
        puts message
  ...
  def prep
    ...
    say "OSS Discovery #{@discovery_version}: Initializing runtime environment...", false
  ...
  def plugin_prep
    unless @@plugin_prepped
      @plugins_list = Hash.new 
      load_plugins

      # make sure all files are writable so we don't scan first and then blow out
      # on writing results
    ...
    # Every property from the config.yml file is loaded as an instance
    # variable of self. This is done so that this file can have default values
    # for all of these properties, and then change them if necessary based on
    # a cli option that was specified.
  ...
  def setup_filters
    ...
    @dir_exclusion_filters["No menu directories"] = '^/usr/share/menu'
    @dir_exclusion_filters["No /media"] = '^/media'
    @dir_exclusion_filters["No man directories"] = '^/usr/share/man'
    ...
    @dir_exclusion_filters["No apt archives"] = '^/var/cache/apt/archives'
  end
...
=end
  ...
  def execute
    ...
    # Rod - 4/23/2009 #@universal_rules_md5 =
    # ScanRulesReader.generate_aggregate_md5(File.dirname(@rules_openlogic))
    @universal_rules_md5 = ScanRulesReader.generate_aggregate_md5("unused")
    @universal_rules_version = ScanRulesReader.get_universal_rules_version 

    # create the application's Walker instance - @list_files is boolean for
    # whether to dump files as encountered
    @walker = Walker::new
  
    if @walker == nil
      say("FATAL - walker cannot be created")
      exit 1
    end
    ...
    @walker.examine_windows_binary_files = @examine_windows_binary_files
    @walker.windows_binary_file_extensions = @windows_binary_file_extensions
    ...
    # create the applications RuleEngine instance in the process of constructing
    # the object, the rule engine will register with the walker and set up the
    # list of files of interest after this object is created, the machine is
    # ready to scan

    unless @list_foi
      say "OSS Discovery is preparing to scan your machine or specified directory."
      say "If the directory or drive being scanned contains many files this will take some time."
      say "You can continue to work on your machine while the scan proceeds."
    end
  
    say "Loading and initializing hand-written signature groups...", false
    start = Time.now
    ScanRulesReader.prep(self)
    @rule_engine = RuleEngine.new(@rules_dirs, @walker, @speed)
    say "done loading #{Utils.number_with_delimiter(@rule_engine.project_rules.size)} in #{Time.now - start} seconds."

    # Load up all our search trees and get ready to find stuff
    SearchTrees.initialize(self)

    # obey the command line parameter to list the files of interest.  this can't
    # be done until the rule engine has parsed the scan rules file so that we
    # know all the actual files of interest determined by scan rules expressions
  
    if ( @list_foi )
      say "Files of interest:"
    ...
    # take a comma delimited --path option and scan each directory
    # indepdendently this lets us handle multiple drives as in:  --path C:,E:,X:
    #
    # or multiple paths such as:
    #
    # --path /usr/bin,/opt/apache2,/opt/local
    #
    # without making the user go through an exclusions filter, or having to
    # enumerate windows drives which is problematic in Ruby for a cli app (pops
    # up unexpected dialogs when accessing drives with no media.)
    ...
    # iterate through directories to scan, and walk each one and accumulate the
    # results
    #
  ...
  def update_scan_rules
    # make sure all proxy information is set before we try to connect to a
    # remote server to get a rule update
    @plugins_list.each do |plugin_name, aPlugin|
  ...
  def validate_directory_to_scan(dir)
    ...
    # --path can now take comma delimited path names, so break it up first, and
    # validate each
    #
    ...
    @directories_to_scan.each do | directory |
      ...
      if ( !File.exist?(directory) )
        ...
        # If it doesn't exist, it may be a weirdism with ruby turning c:\ into
        # /c:/.  So make that change and try again
      ...
      unless dir_exists
        say "The given path to scan does not exist: #{directory}"
        return false
      end
  ...
  # Do a little trick to get the 'real' local IP address.  The trick is to
  # create a UDPSocket that intends to connect to the openlogic.com IP address.
  # It doesn't actually connect, but forces the networking stack to figure out
  # which local IP address is actually bound for external traffic.  We then ask
  # for that IP address.
  def get_local_ip
  ...
  def make_reports
    ...
    @plugins_list.each do | plugin_name, aPlugin |
      if aPlugin.respond_to?(:report)
        # human readable report
        unless @olex_human_results_file.nil? || @olex_human_results_file.empty?
          report_file_name = @olex_human_results_file
        else
          report_file_name = aPlugin.local_report_filename
        end

        unless @show_rollup_report.nil?
          if aPlugin.respond_to?(:show_rollup)
            aPlugin.show_rollup = @show_rollup_report
          end
        end

        unless @olex_rollup_results_file.nil? || @olex_rollup_results_file.empty?
          rollup_report_file_name = @olex_rollup_results_file
        else
          rollup_report_file_name = aPlugin.local_rollup_report_filename
        end

        say "\nWriting the scan results to file '#{report_file_name}'."
        say "Writing the scan rollup results to file '#{rollup_report_file_name}'."

        aPlugin.report(report_file_name, @packages, @scandata, rollup_report_file_name)
      end

      # if the plugin will respond to a machine report method, fire it off
      if aPlugin.respond_to?(:machine_report)
        unless @olex_machine_results_file.nil? || @olex_machine_results_file.empty?
          report_file_name = @olex_machine_results_file
        else
          report_file_name = aPlugin.machine_report_filename
        end

        say "\nWriting the machine scan results to file '#{report_file_name}'."

        aPlugin.machine_report(report_file_name, @packages, @scandata)

        if @preview_results && report_file_name != STDOUT
          say "\nThese are the actual machine scan results from the file, #{report_file_name}, that would be delivered by --deliver-results option"
          say File.new(report_file_name).read
        end
      end

      # if the plugin will respond to a mif report method, fire it off
      if aPlugin.respond_to?(:mif_report)
        unless @olex_mif_results_file.nil? || @olex_mif_results_file.empty?
          report_file_name = @olex_mif_results_file
        else
          report_file_name = aPlugin.mif_report_filename
        end

        say "\nWriting the MIF scan results to file '#{report_file_name}'."

        aPlugin.mif_report(report_file_name, @packages, @scandata)

        if @preview_results && report_file_name != STDOUT
          say "\nThese are the actual machine scan results from the file, #{report_file_name}, that can be imported into Tivoli Inventory."
          say File.new(report_file_name).read
        end
      end
  ...
  def process_args
    ...
    options_array << [ "--windows-binary-scan", "-w", GetoptLong::NO_ARGUMENT ]   # look inside windows binary files for DLL's
    ...
    begin
      ...
      options.each do | opt, arg |
        ...
        when "--conf"
          if ( File.exist?(arg) && File.file?(arg) )
            ...
            say "The given configuration path does not exist or is not a file: #{arg}"
            exit 1
          end
  
        when "--deliver-batch"
          if ( !File.directory?(arg) )
            say "#{arg} does not exist, please recheck the directory name"
            exit 1
          end
          ...
          # existence says 'yes' deliver the machine readable results to the
          # server optional arg will either immediately deliver results if the
          # file already exists or will scan the machine and use that filename
          # as the results file and then deliver it if no results filename is
          # given, the machine will be rescanned and results placed in the
        ...
        when "--deliver-results"
          ...
          if ( arg != nil && arg != "" )
            # results file was given, see if it exists. if it exists, post it
            # immediately, exit once the status code is received from the server
            # if it does not exist, scan the machine normally except use the
            # given filename as the the results file to post when the scan is
            # complete

            if ( File.exists?(arg) )
              @immediate_filename = arg
            else
              say "The file, #{@immediate_filename} does not exist."
              say File.expand_path(arg)
              exit 1
            end
          end
        ...
        when "--list-os"
          say "#{get_os_version_str()}, arch: #{@os_architecture}, kernel: #{@kernel}"
          exit 0
        ...
        when "--list-projects"
          ...
          if (arg == "verbose") then
            say "number,name,from,platforms,description"
            projects.each_with_index do |p, i|
              say "#{i+1},#{p.name},#{p.from},#{p.operating_systems.to_a.inspect.gsub(", ", "|")},#{p.desc}"
            end
          else
        ...
        when "--list-tag"
          say "Unique Machine Tag (ID): #{@machine_id}"
          exit 0
        ...
        when "--update-rules"
          if (arg == nil || arg == "") then
            ...
            say "The only valid arg for the '--update-rules' option is 'scan'. You provided an arg of '#{arg}'."
            exit 1
          end
        ...
        when "--windows-binary-scan"
          if Utils.major_platform =~ /windows/
            @examine_windows_binary_files = true
          else
            say "The --windows-binary-scan switch is only available when running OSS Discovery on a Windows platform - ignoring it."
          end

        end   # case

        # now give each plugin a whack at any command line option that was
        # offered so it can collect additional state if needs to, not just
        # processing it's own specific options
      ...
      if (e.is_a?(GetoptLong::InvalidOption)) then
        say "Unsupported option. Please review the list of supported options and usage:"
        ...
        say "Unexpected error (#{e.message}). Please see the log for more detailed information."
        @@log.error('Discovery') {e.inspect + e.backtrace.inspect.gsub("[\"", "\n\t[\"").gsub(", ", ",\n\t ")}
        help()
        exit 1
      end
    ...
    if defined? @immediate_filename
      say "Immediately delivering the results file: #{@immediate_filename}..."

      # plugins are responsible for determining if this file is of the type it
      # can
      @plugins_list.each do | plugin_name, aPlugin |
  ...
  def run
    # #----------------------------- do the business -------------------------------------
    # clear these values in case discovery is run multiple times
    @rule_engine = nil
    @walker = nil
    @packages = nil
    ClassFileArchiveDiscoverer.reset
    SourceFileDiscoverer.reset
    WindowsBinaryFileDiscoverer.reset

    # If this is running under jruby, we ignore the --nofollow preference and
    # manually set symlinks to not be followed.  Jruby has a lot of problems
    # with symlinks, so we have to completely ignore them unless running in
    # native ruby.
    @follow_symlinks = false if RUBY_PLATFORM =~ /java/


    # test access to the destination_url for posting to make sure we can get out
    # ok pre-check and warn if server cannot be reached
    if @send_results && !@@already_checked_connectivity
      @@already_checked_connectivity = true
      @plugins_list.each do |plugin_name, aPlugin|
        if aPlugin.can_deliver? && (check_network_connectivity(aPlugin.destination_server_url) == false) # checks to see if scan results post server is reachable.
          ...
          say "The scan will continue, but you will need to manually post your scan results.\n"
          say "The URL used to contact the server is: #{aPlugin.destination_server_url}"
          if ( aPlugin.upload_url != nil )
            say "Manual upload URL: #{aPlugin.upload_url}\n\n"
          end
          say "Could not reach destination URL: #{aPlugin.destination_server_url}"
        end
      end
    end

    if @update_rules && !@dont_update_rules && !@@already_checked_for_updates
      @@already_checked_for_updates = true
      ...
      begin
        ...
        if e.message =~ /connection refused/i
          say "\n\n"
          say "Could not connect to the server at #{@server_base_url} to check for"
          say "updated rule files."
          say "If you are behind a proxy, you may want to update the proxy values"
          say "found in the configuration .yml file under the plugins conf directory."
          say "If you already have proxy values set correctly, it's possible that"
          say "the discovery server is down at the moment.  Please try again later."
          say ""
        else
          say e.message
        end
    ...
    # give plugins a chance to send their results somewhere
    send_scan_results
  ...
  def send_scan_results(results_file_name = nil)
    result = nil
    if @send_results
      @plugins_list.each do |plugin_name, aPlugin|
        if aPlugin.can_deliver?
          result = aPlugin.send_results(results_file_name)
        end
      end
    end
    result
