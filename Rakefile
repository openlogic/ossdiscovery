...
namespace :release do
  ...
  ENV['plugin'] = ENV['plugin'].nil? ? "olex" : ENV['plugin']
  PACKAGE_NAME= ( ENV["NAME"] || "ossdiscovery" ) + "-" + ENV['plugin']
  PACKAGE_VERSION=ENV["VERSION"] || "2.3.3"
  ...
  namespace :ruby do
    ...
    Rake::PackageTask.new(PACKAGE_NAME, PACKAGE_VERSION) do |p|
      ...
      p.need_tar_gz = true
      p.need_zip = true
      p.package_files.include("lib/**/*", "doc/*", "log/*", "log", "LICENSE", "NOTICE", "README", "discovery", "discovery.bat", "help.txt" )
  ...
  namespace :jruby do
    ...
    # OLEX is the default plugin
      ...
      cp_r "3rdparty", "pkg/#{jruby_package_filename}/3rdparty", :remove_destination=>true
      ...
      cp "LICENSE", "pkg/#{jruby_package_filename}/"
      cp "NOTICE", "pkg/#{jruby_package_filename}/"
      cp "README", "pkg/#{jruby_package_filename}/"
  ...
  namespace :windows do
    ...
    task :prepare => "release:windows:clean" do
      ...
      if ( plugin == "inventory" )
        ...
        rm_r "pkg/#{package_filename}/lib/plugins/olex"
      # OLEX is now the default plugin
      #elsif ( plugin == "olex")
      else
        rm_r "pkg/#{package_filename}/lib/plugins/inventory"
        rm_r "pkg/#{package_filename}/lib/plugins/census"
      ...
      cp_r "3rdparty", "#{dest_dir}/3rdparty", :remove_destination=>true
      cp_r "log", "#{dest_dir}/log", :remove_destination=>true
      cp_r "doc", "#{dest_dir}/doc", :remove_destination=>true
      cp_r "jre/jre-1.6.0_20-windows-ia32", "#{dest_dir}/jre", :remove_destination=>true
      cp "LICENSE", "#{dest_dir}"
      cp "NOTICE", "#{dest_dir}"
      cp "README", "#{dest_dir}"
  ...
  namespace :linux do
    ...
    task :prepare => "release:linux:clean" do
      ...
      cp_r "log", "#{dest_dir}/log", :remove_destination=>true
      cp_r "doc", "#{dest_dir}/doc", :remove_destination=>true
      cp_r "jre/jre-1.6.0_20-linux-ia32", "#{dest_dir}/jre", :remove_destination=>true
      cp "LICENSE", "#{dest_dir}"
      cp "NOTICE", "#{dest_dir}"
      cp "README", "#{dest_dir}"
      cp "help.txt", "#{dest_dir}"
