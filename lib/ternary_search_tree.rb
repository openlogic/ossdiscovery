...
# efficiently load file name rules and then search against them with file names.
class TernarySearchTree
  ...
  # character is actually stored as its integer value to make comparisons fast.
  def get(key)
    # start at the root
    node = get_node(@root)
    ...
    for i in 0...key.size
      ...
      # character we're looking for.  This means that if we're not at the end of
      # the entire key string, we need to continue from the 'equal' child.
      if i + 1 != key.length
    ...
    # We're now at the final node in the chain that defines the given key string
    # so it's time to retrieve our value.  Note that this could still be nil
    # because that's what's explicitly stored here or because the string we're
    # looking for is a subset of some other key with a value further down the
    # tree.
    node[VALUE]
  end
  ...
  def put(key, value)
    ...
    for i in 0...key.size
      ...
      # To get through the loop above, the current node must now contain the
      # character we're looking for.  This means that if we're not at the end of
      # the entire key string, we need to create a new 'equal' child node and
      # proceed from there.
      if i + 1 != key.length
    ...
    # we're now at the final node in the chain that defines the given key string
    # so it's time to store our value
    node[VALUE] = value
  end
  ...
  # Return the value associated with the key matching the given file name or nil
  # if none is found.  The definition of 'match' is based on this algorithm:
  #
  ...
  def match(filename)
    ...
    for i in 0...filename.size
      ...
      if node
        ...
        # couldn't find the character, so we're done with the tree. look at the
        # rest of the file name to see if we're already up to the version
  ...
  def load_from_file(file_name)
    ...
    IO.foreach(file_name) do |line|
      begin
        package_id, name, aliases, languages = line.split(PROJECT_DELIMITER)
...
ProjectInfo = Struct.new(:package_id, :package_name, :language_symbols) unless defined? ProjectInfo